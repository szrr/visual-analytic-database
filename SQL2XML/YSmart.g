...
identifier
	:	VECTOR  // add_new_grammar
	|	PATH   // add_new_grammar
	|   ID
	|	DOUBLEQUOTED_STRING 
   	;
...
table_reference_list
	:	(
			((table_name t_alias? kNN_join_clause)|join_clause|(LPAREN join_clause RPAREN)|table_reference)
			(COMMA ((table_name t_alias? kNN_join_clause)|join_clause|(LPAREN join_clause RPAREN)|table_reference))*
		)
	;            
table_reference
...
outer_join_sign
	...
	;
kNN_join_clause
	:	kNN_join_type k_join table_reference ( k_on ID EQ ID )?
	;
kNN_join_type
	:	'KNN' LPAREN 'K=' NUMBER RPAREN
	;
table_name
	:	sql_identifier
	;
...
k_distance : r='DISTANCE' { $r.setType(T_RESERVED) }  ;  // add_new_grammar
k_video_extraction : r='VIDEO_EXTRACTION' { $r.setType(T_RESERVED) }  ;  // add_new_grammar
k_extraction : r='EXTRACTION' { $r.setType(T_RESERVED) }  ;  // add_new_grammar
k_video_feature_extraction : r='VIDEO_FEATURE_EXTRACTION' { $r.setType(T_RESERVED) }  ;  // add_new_grammar
k_knn_avg_distance : r='kNN_AVG_DISTANCE' { $r.setType(T_RESERVED) }  ;  // add_new_grammar
k_knn : r='KNN' { $r.setType(T_RESERVED) }  ;  // add_new_grammar
k_outlier : r='OUTLIER' { $r.setType(T_RESERVED) }  ;  // add_new_grammar
k_farthest : r='FARTHEST' { $r.setType(T_RESERVED) }  ;  // add_new_grammar
k_feature_distance : r='FEATURE_DISTANCE' { $r.setType(T_RESERVED) }  ;  // add_new_grammar
k_outlier_line : r='OUTLIER_LINE' { $r.setType(T_RESERVED) }  ;  // add_new_grammar
k_outlier_score : r='OUTLIER_SCORE' { $r.setType(T_RESERVED) }  ;  // add_new_grammar
k_video_similarity : r='VIDEO_SIMILARITY' { $r.setType(T_RESERVED) }  ;  // add_new_grammar
...
keyword
    ...
    | 'DEPTH'| 'DISTANCE'    // add_new_grammar
	| 'EACH'	| 'ENABLE'	| 'END'	| 'ESCAPE'	| 'EVENTS'	| 'ERRORS'	| 'EXCEPT'	| 'EXCEPTIONS'	| 'EXEC'	| 'EXECUTE'
    | 'EXCLUDE' | 'EXTRACTION'   // add_new_grammar
	| 'EXPLAIN'	| 'EXTENT'	| 'EXTERNALLY'	
	| 'FETCH'	| 'FIRST'	| 'FLUSH'	| 'FORCE'	| 'FOREIGN'	| 'FORTRAN'	| 'FOUND' | 'FARTHEST'
	| 'FOLLOWING'    | 'FREELIST' | 'FEATURE_DISTANCE'
	| 'FREELISTS'	| 'FUNCTION'	
	| 'GO'	| 'GOTO'	| 'GROUPS'
	| 'INCLUDE'	| 'INCLUDING'	| 'INDICATOR'	| 'INITRANS'	| 'INSTANCE'	| 'INT'	
	| 'KEY'	| 'KNN' | 'KNN_AVG_DISTANCE' //add_new_grammar
	...
	| 'OFF'	| 'OLD'	| 'ONLY'	| 'OPEN'	| 'OPTIMAL'	| 'OWN'	| 'OUTLIER' | 'OUTLIER_LINE' | 'OUTLIER_SCORE'  // add_new_grammar
    ...
    | 'VALUE'	| 'VERSIONS' | 'VIDEO_FEATURE_EXTRACTION' | 'VIDEO_SIMILARITY' | 'VIDEO_EXTRACTION'   // add_new_grammar
	| 'WHEN'	| 'WORK'	| 'WRITE'
   	| 'YEAR'
	| 'ZONE'
    ...
    | 'NVARCHAR'	| 'NVARCHAR2'	|    'OBJECT_VALUE'
    | 'ORA_ROWSCN'
    | 'OVER'
    | 'PIVOT'
