...
preload_threshold = 1024 * 1024 * 1024 * 100
loaded_table_list = {}
cols_to_index = []
...
def __get_gb_exp__(exp,tmp_list):
    ...
    if exp.func_name in ["SUM","AVG","COUNT","MAX","MIN","VIDEO_SIMILARITY","OUTLIER_LINE","OUTLIER_SCORE"]:
        tmp_list.append(exp)
    else:
...
def type_length(tn, colIndex, colType, past_tn = None):
    if colType in ["INTEGER", "DATE"]:
        return "sizeof(int)"
    elif colType in ["TEXT"]:
        if tn in ["LEFT", "RIGHT"]:
            # colLen = schema[past_tn].column_list[colIndex].column_others
            # colLen = schema[past_tn].column_list[1].column_others
            colLen = 23
        else:
        ...
        colLen = schema[tn].column_list[colIndex].column_others
        return str(int(colLen)*4)

"""
Get the exp information from the where expresion.
"""

def get_where_attr(exp, whereList, relList, conList, extractionList = [], extractionPath = []):
    if isinstance(exp, ystree.YFuncExp):
        if exp.func_name in ["AND", "OR"]:
            ...
            if exp.func_name in ["LEQ"] and isinstance(exp.parameter_list[0], ystree.YFuncExp) and \
                exp.parameter_list[0].func_name in ["KNN"]:  # LEQ '<='
                # e.g. kNN(VIDEO_EXTRACTION(path"/home/szr/subquery/video_process/test_video/6578103253609745667.mp4"), d_feature) <= 5
                relList.append(exp.parameter_list[0].func_name) #KNN
                conList.append(exp.parameter_list[1].cons_value) # 5
                whereList.append(exp.parameter_list[0].parameter_list[1]) # d_feature column
                extractionList.append(exp.parameter_list[0].parameter_list[0].func_name) # VIDEO_EXTRACTION
                extractionPath.append(exp.parameter_list[0].parameter_list[0].parameter_list[0].cons_value) # path
            else:
                relList.append(exp.func_name)
                extractionList.append("0")
                extractionPath.append("0")
                for x in exp.parameter_list:
                    if isinstance(x, ystree.YRawColExp):
                        ...
                        conList.append(x)
                    # elif isinstance(x, ystree.YFuncExp) and x.func_name == "DISTANCE":
                    #     relList.append(x.func_name)
                    #     whereList.append(x.parameter_list[0])
                    #     whereList.append(x.parameter_list[1])
                    # elif isinstance(x, ystree.YFuncExp) and x.func_name in ["LEQ", "LTH"]: #LTH '<' , LEQ '<='
                    #     relList.append(x.func_name)
                    #     whereList.append(x.parameter_list[0])
                    #     whereList.append(x.parameter_list[1])
...
def printMathFunc(fo,prefix, mathFunc):
    ...
    if mathFunc.opName == "COLUMN":
        ...
        print >>fo, prefix + ".opValue = " + str(mathFunc.value) + ";"
    elif mathFunc.opName == "COLUMN_VECTOR":
        print >>fo, prefix + ".op = NOOP;"
        print >>fo, prefix + ".opNum = 1;"
        print >>fo, prefix + ".exp = 0;"
        print >>fo, prefix + ".opType = COLUMN_VECTOR;"
        print >>fo, prefix + ".opValue = " + str(mathFunc.value) + ";"
        ...
        print >>fo, prefix + ".opValue = " + str(mathFunc.value) + ";"
    elif mathFunc.opName == "DISTANCE":
        print >>fo, prefix + ".op = DISTANCE;"
...
def generate_code(tree):
    ...
    if CODETYPE == 0:
        ...
        print >>fo, "#include <faiss/gpu/GpuAutoTune.h>"
        print >>fo, "#include <faiss/gpu/GpuCloner.h>"
        print >>fo, "#include <faiss/gpu/GpuIndexIVFPQ.h>"
        print >>fo, "#include <faiss/gpu/StandardGpuResources.h>"
        print >>fo, "#include <faiss/index_io.h>"
        print >>fo, "#include <faiss/IndexFlat.h>"
        print >>fo, "#include <faiss/IndexIVFPQ.h>"
        print >>fo, "extern struct tableNode* tableScan(struct scanNode *,struct statistic *, Mempool *, Mempool *, Mempool *, int *, int *, int *, faiss::Index*);"
        #Indexing functions
        ...
        if joinType == 0:
            ...
            print >>fo, "extern struct tableNode* knnJoin(struct joinNode *, struct statistic *, Mempool *, Mempool *, Mempool *, int *, faiss::Index*);"
            print >>fo, "extern struct tableNode* cartesianProductJoin(struct joinNode *, struct statistic *, Mempool *, Mempool *, Mempool *, int *);"
        ...
        if joinType == 0:
            print >>fo, "extern struct tableNode* knnJoin(struct joinNode *, struct clContext *, struct statistic *);"
            print >>fo, "extern struct tableNode* hashJoin(struct joinNode *, struct clContext *, struct statistic *);"
            print >>fo, "extern struct tableNode* cartesianProductJoin(struct joinNode *, struct clContext *, struct statistic *);"
        else:
    ...
    if optimization != None and "--idx" in optimization: # add new grammar
    ...
    #For sub profiling
    print >>fo, indent + "pp.outerTableSize = pp.resultTableSize = pp.subCacheHits = 0;\n"

    #For ANNs search
    print >>fo, indent + "pp.annsTotalTime = 0;"
    print >>fo, indent + "pp.loadIndex = 0;"
    print >>fo, indent + "pp.indexCPU2GPU = 0;"
    print >>fo, indent + "pp.trainIndex = 0;"
    print >>fo, indent + "pp.saveIndex = 0;"
    print >>fo, indent + "pp.indexAddData = 0;"
    print >>fo, indent + "pp.indexSearch = 0;"
    ...
    #For join profiling
    print >>fo, indent + "pp.knnJoin_totalTime = 0;"
    print >>fo, indent + "pp.knn_ANNs_time = 0;"

    print >>fo, indent + "pp.cartesianJoin_totalTime = 0;"
    ...
    for table,node in pre_executed_nodes.items():
        if isinstance(node.parent, ystree.TwoJoinNode):
            ...
            if leftIndex is None and rightIndex is None:
                print "JoinType: CartesianProduct"
                leftIndex = rightIndex = -1
            
            elif leftIndex == None or rightIndex == None:
    ...
    print >>fo, indent + "printf(\"<--ANNs total time-->          : %lf\\n\", pp.annsTotalTime/(1000*1000));\n"
    print >>fo, indent + "printf(\"Load anns index                : %lf\\n\", pp.loadIndex/(1000*1000));\n"
    print >>fo, indent + "printf(\"Load index from CPU to GPU     : %lf\\n\", pp.indexCPU2GPU/(1000*1000));\n"
    print >>fo, indent + "printf(\"Train anns index               : %lf\\n\", pp.trainIndex/(1000*1000));\n"
    print >>fo, indent + "printf(\"Save anns index                : %lf\\n\", pp.saveIndex/(1000*1000));\n"
    print >>fo, indent + "printf(\"Add data to anns index         : %lf\\n\", pp.indexAddData/(1000*1000));\n"
    print >>fo, indent + "printf(\"Search anns index              : %lf\\n\", pp.indexSearch/(1000*1000));\n"

    print >>fo, indent + "printf(\"<---SUB()--->\\n\");"
    print >>fo, indent + "printf(\"Outer table size           : %d\\n\", pp.outerTableSize);\n"
    print >>fo, indent + "printf(\"Sub Cache hits             : %d\\n\", pp.subCacheHits);\n"
    ...
    print >>fo, indent + "printf(\"<---Join--->\\n\");"
    print >>fo, indent + "printf(\"Calls                     : %d\\n\", pp.join_callTimes);\n"
    print >>fo, indent + "printf(\"<---knnJoin()--->\\n\");"
    print >>fo, indent + "printf(\"Total time                : %lf\\n\", pp.knnJoin_totalTime/(1000*1000));"
    print >>fo, indent + "printf(\"Total ANNs time           : %lf\\n\", pp.knn_ANNs_time/(1000*1000));"
    print >>fo, indent + "printf(\"<---cartesianJoin()--->\\n\");"
    ...
    print >>fo, indent + "printf(\"Step 4.1 - Materialize left part (joinFact)       : %lf\\n\", pp.joinProf_step41_materialize_res_left/(1000*1000));"
    print >>fo, indent + "printf(\"Step 4.2 - Materialize right part (joinDim)       : %lf\\n\", pp.joinProf_step42_materialize_res_right/(1000*1000));"
    print >>fo, indent + "printf(\"<----------------->\");"
    print >>fo, indent + "printf(\"\\n\");"
...
def generate_code_for_a_subquery(fo, lvl, rel, con, tupleNum, tableName, indexDict, currentNode, passInPos):
    ...
    if rel in ["EQ", "LTH", "GTH", "LEQ", "GEQ", "NOT_EQ"]:
        ...
        subq_res_size = type_length(subq_res_col.table_name, subq_res_col.column_name, subq_res_col.column_type, subq_res_col.past_table_name)
    ...
    correlated_pairs = []
    if optimization!= None and "--idx" in optimization:
    ...
    if rel != "IN":
        ...
        #Add loop info
        print >>fo, indent + baseIndent * 2 + "pp.outerTableSize = " + (subq_res_size if constant_len_res else tableName+"->tupleNum" ) + ";"
        ...
        if SubCache == 1 and len(pass_in_cols) == 1:
            ...
            print >>fo, indent + "pp.subCacheHits = pp.subCacheHits + 1;"
            print >>fo, indent + baseIndent + "}"
        ...
        if constant_len_res:
            print >>fo, indent + baseIndent + "mempcpy(" + var_subqRes + " + 0 * " + subq_res_size + ", final, " + subq_res_size + ");"
            if SubCache == 1 and len(pass_in_cols) == 1:
                col = pass_in_cols[0]
                pass_in_var = "_" + col.table_name + "_" + str(col.column_name)
                print >>fo, indent + baseIndent + "cache[*(" + to_ctype(col.column_type).lower() + " *)" + pass_in_var + "] = *(float *)final;"
        else:
            print >>fo, indent + baseIndent + "((char **)" + var_subqRes + ")[0] = (char *)malloc(sizeof(int) + " + subq_res_size + " * mn.table->tupleNum);"
            print >>fo, indent + baseIndent + "CHECK_POINTER( ((char **)" + var_subqRes + ")[0] );"
            print >>fo, indent + baseIndent + "*(int *)(((char **)" + var_subqRes + ")[0]) = mn.table->tupleNum;"
            print >>fo, indent + baseIndent + "mempcpy(((char **)" + var_subqRes + ")[0] + sizeof(int), final, " + subq_res_size + " * mn.table->tupleNum);"

        if HostMempool == 1:
            print >>fo, indent + baseIndent + "host_mempool.freeto(free_pos);"
        if SubResMempool == 1:
            print >>fo, indent + baseIndent + "gpu_inter_mp.freeto(free_gpu_pos);"

        #Add loop info
        print >>fo, indent + baseIndent + "pp.outerTableSize = "+tableName+"->tupleNum;"
        print >>fo, indent + baseIndent + "pp.resultTableSize = mn.table->tupleNum;"
...
def generate_code_for_a_select_project_node(fo, indent, lvl, spn):
    ...
    if spn.table_name != None:
        ...
        #     return tn.name
        resName = spn.name
        tnName = spn.table_name.lower() + "Table"
        var_subqRes = "subqRes" + str(lvl)
        ...
        relName = "selectProjectRel"
        print >>fo, indent + "struct scanNode " + relName + ";"
        print >>fo, indent + relName + ".tn = " + inputNode + ";"
        
        selectList = spn.select_list.tmp_exp_list

        expList = []
        def get_exps(exp, expList):
            if isinstance(exp, ystree.YFuncExp):
                if exp.func_name in ["AND", "OR"]:
                    for x in exp.parameter_list:
                        if isinstance(x, ystree.YFuncExp):
        ...
        if spn.where_condition != None:
            get_exps(spn.where_condition.where_condition_exp, expList)
    ...
    if spn.where_condition is not None:
        ...
        get_where_attr(spn.where_condition.where_condition_exp, whereList, relList, conList, extractionList, extractionPath)
        newWhereList = []
        ...
        if spn.where_condition.where_condition_exp.func_name in ["AND","OR"]:
            print >>fo, indent + "(" + relName + ".filter)->andOr = " + spn.where_condition.where_condition_exp.func_name + ";"
        else:
        ...
        for i in range(0, len(whereList)):
            ...
            if isinstance(conList[i], ystree.YFuncExp) and conList[i].func_name == "SUBQ":
                ...
                for j in range(0, len(indexList)):
                    indexDict[ spn.table_name + "." + str(indexList[j])] = j
            ...
            if isinstance(conList[i], ystree.YFuncExp) and conList[i].func_name == "SUBQ":
                ...
                if relList[i] in ["KNN"]:
                    ...
                    print >>fo, indent + "}"
                    # extract vectors
                    print("[VIDEO_EXTRACTION] start")
                    if extractionList[i] == "VIDEO_EXTRACTION":
                        subprocess.call(["python3", "../../vector_src/init_vector/video_feature_extraction.py", \
                                                        str(extractionPath[i]), "../../vector_src/init_vector/videoFeatures"])
                    print("[VIDEO_EXTRACTION] end")
                    # pass vector path
                    print >>fo, indent + "{"
                    print >>fo, indent + baseIndent + "char *tmp = \"../../vector_src/init_vector/videoFeatures\";"
                    print >>fo, indent + baseIndent + "strcpy(("+ relName + ".filter)->exp[" + str(i) + "].videoFeaturePath, tmp);"
                    print >>fo, indent + "}"
            elif ctype == "INT":
        ...
        ############## end of wherecondition not none
            ...
            if DeviceMempool == 1:
                ...
                print >>fo, indent + resName + " = tableScan(&" + relName + ", &pp, " + h_mp + ", " + d_inner_mp + ", "  + d_res_mp + ", NULL, NULL, NULL, NULL);"
...
def generate_code_for_a_group_by_node(fo, indent, lvl, gbn):
    ...
    for i in range(0,selectLen):
        exp = select_list[i]
        if isinstance(exp, ystree.YFuncExp) and exp.func_name == "OUTLIER_LINE":
            ...
            print >>fo, indent + "gbNode->gbExp["+str(i)+"].index = " + str(exp.parameter_list[0].column_name) + ";"
        
        elif isinstance(exp, ystree.YFuncExp) and exp.func_name == "OUTLIER_SCORE":
            print >>fo, indent + "gbNode->tupleSize += sizeof(float);"
            print >>fo, indent + "gbNode->attrType[" + str(i) + "] = FLOAT;"
            print >>fo, indent + "gbNode->attrSize[" + str(i) + "] = sizeof(float);"
            print >>fo, indent + "gbNode->gbExp["+str(i)+"].func = " + exp.func_name + ";"
        ...
        elif isinstance(exp, ystree.YFuncExp) and exp.func_name == "SUM" \
            and isinstance(exp.parameter_list[0], ystree.YFuncExp) \
                and exp.parameter_list[0].func_name == "DISTANCE":
            print >>fo, indent + "gbNode->tupleSize += sizeof(float);"
            print >>fo, indent + "gbNode->attrType[" + str(i) + "] = FLOAT;"
            print >>fo, indent + "gbNode->attrSize[" + str(i) + "] = sizeof(float);"
            print >>fo, indent + "gbNode->gbExp["+str(i)+"].func = " + exp.func_name + ";"
            print >>fo, indent + "gbNode->gbExp["+str(i)+"].index = " + str(exp.parameter_list[0].parameter_list[0].column_name) + ";"
            prefix = indent + "gbNode->gbExp[" + str(i) + "].exp"
            print >>fo, prefix + ".op = NOOP;"
            print >>fo, prefix + ".opNum = 1;"
            print >>fo, prefix + ".exp = 0;"
            print >>fo, prefix + ".opType = COLUMN_DECIMAL;"
            print >>fo, prefix + ".opValue = " + str(exp.parameter_list[0].parameter_list[0].column_name) + ";"
        

        elif isinstance(exp, ystree.YFuncExp) and exp.func_name == "VIDEO_SIMILARITY":
            # case1 : VIDEO_SIMILARITY(VIDEO_EXTRACTION(path"xxx.mp4"), d_feature)
            if exp.parameter_list[0].func_name == "VIDEO_EXTRACTION":
                # extract vectors
                videoExtractionExp = exp.parameter_list[0]
                pathExp = videoExtractionExp.parameter_list[0]
                print("[VIDEO_EXTRACTION] start")
                subprocess.call(["python3", "../../vector_src/init_vector/video_feature_extraction.py", \
                                                str(pathExp.cons_value), "../../vector_src/init_vector/videoFeatures"])
                print("[VIDEO_EXTRACTION] end")
                # pass vector path
                print >>fo, indent + "{"
                print >>fo, indent + baseIndent + "char *tmp = \"../../vector_src/init_vector/videoFeatures\";"
                print >>fo, indent + baseIndent + "strcpy(gbNode->gbExp[" + str(i) + "].videoFeaturePath, tmp);"
                print >>fo, indent + "}"
                ...
                print >>fo, indent + "gbNode->attrSize[" + str(i) + "] = sizeof(float);"
                print >>fo, indent + "gbNode->gbExp["+str(i)+"].func = " + exp.func_name + ";"
                print >>fo, indent + "gbNode->gbExp["+str(i)+"].index = " + str(exp.parameter_list[1].column_name) + ";"
                print >>fo, indent + "gbNode->gbExp["+str(i)+"].type = 0;"
            
            # case2 : VIDEO_SIMILARITY(DISTANCE(s_feature, d_feature))
            elif exp.parameter_list[0].func_name == "DISTANCE":
                # feature column
                print >>fo, indent + "gbNode->tupleSize += sizeof(float);"
                print >>fo, indent + "gbNode->attrType[" + str(i) + "] = FLOAT;"
                print >>fo, indent + "gbNode->attrSize[" + str(i) + "] = sizeof(float);"
                print >>fo, indent + "gbNode->gbExp["+str(i)+"].func = " + exp.func_name + ";"
                print >>fo, indent + "gbNode->gbExp["+str(i)+"].index = " + str(exp.parameter_list[0].parameter_list[0].column_name) + ";"
                print >>fo, indent + "gbNode->gbExp["+str(i)+"].type = 1;"
                print >>fo, indent + "gbNode->gbExp["+str(i)+"].searchFrameNum = searchFrameNum;"
            ...
            colIndex = exp.column_name
            print >>fo, indent + "gbNode->attrType[" + str(i) + "] = gbNode->table->attrType[" + str(colIndex) + "];"
            print >>fo, indent + "gbNode->attrSize[" + str(i) + "] = gbNode->table->attrSize[" + str(colIndex) + "];"
            print >>fo, indent + "gbNode->tupleSize += gbNode->table->attrSize[" + str(colIndex) + "];"
...
def generate_code_for_a_two_join_node(fo, indent, lvl, jn):
    ...
    # generate column indices that need to be extracted from TableNode
    # generate l/rOutList that map TwoJoinNode select list
    ...
    if leftIndex is None and rightIndex is None:
        print "JoinType: CartesianProduct"
        leftIndex = rightIndex = -1
    elif leftIndex is None or rightIndex is None:
    ...
    if CODETYPE == 0:
        ...
        if jn.join_type == 'KNN':
            print >>fo, indent + jName + ".nearestk = " + str(jn.join_parameter) + ";"
            # print >>fo, indent + "joinRes = knnJoin(&" + jName + ", &pp, " + h_mp + ", " + d_inner_mp + ", " + d_res_mp + ", " + (rightName + "_hash" if lvl > 0 else "NULL") + ");"
            print >>fo, indent + "joinRes = knnJoin(&" + jName + ", &pp, " + h_mp + ", " + d_inner_mp + ", " + d_res_mp + ", " + "NULL" + ", NULL);"
        elif leftIndex == -1 and rightIndex == -1:
            print >>fo, indent + "searchFrameNum = (" + jName + ".leftTable)->tupleNum;"
            print >>fo, indent + "joinRes = cartesianProductJoin(&" + jName + ", &pp, " + h_mp + ", " + d_inner_mp + ", " + d_res_mp + ", " + "NULL" + ");"
        else:
            # print >>fo, indent + "joinRes = hashJoin(&" + jName + ", &pp, " + h_mp + ", " + d_inner_mp + ", " + d_res_mp + ", " + (rightName + "_hash" if lvl > 0 else "NULL") + ");"
            print >>fo, indent + "joinRes = hashJoin(&" + jName + ", &pp, " + h_mp + ", " + d_inner_mp + ", " + d_res_mp + ", " + "NULL" + ");"
        if DeviceMempool == 1:
    ...
    scanExist = 0
    if (jn.where_condition is not None and jn.where_condition != [] and \
                (not hasattr(jn.join_condition, 'where_condition_exp') or \
                not jn.where_condition.where_condition_exp.compare(jn.join_condition.where_condition_exp))) or \
                (jn.project_func_list is not None and jn.project_func_list != []):
        scanExist = 1
        expList = []
        ...
        if jn.where_condition != None:
            ...
            print >>fo, indent + relName + ".hasWhere = 0;"
            print >>fo, indent + relName + ".whereAttrNum = " + str(len(expList)) + ";"
            if HostMempool == 1:
                print >>fo, indent + relName + ".whereIndex = (int *)host_mempool.alloc(sizeof(int) * " + str(len(expList)) + ");"
                print >>fo, indent + relName + ".outputIndex = (int *)host_mempool.alloc(sizeof(int) * " + str(len(selectList)) + ");"
        ...
        for i in range(0, len(expList)):
            ...
            left_col = None
            right_col = None
            if(isinstance(exp.parameter_list[0], ystree.YFuncExp) and exp.parameter_list[0].func_name == "DISTANCE"):
                left_col = exp.parameter_list[0].parameter_list[0]
                right_col = exp.parameter_list[0].parameter_list[1]
            else:
        ...
        if jn.where_condition != None:
            ...
            if HostMempool == 1:
                ...
                print >>fo, indent + "CHECK_POINTER((" + relName + ".filter)->exp);"
            if jn.where_condition != None and jn.where_condition.where_condition_exp.func_name in ["AND","OR"]:
        ...
        for i in range(0, len(expList)):
            ...
            if(isinstance(exp.parameter_list[0], ystree.YFuncExp) and exp.parameter_list[0].func_name == "DISTANCE"):
                ...
                colType = right_col.column_type
                ctype = to_ctype(colType)

                print >>fo, indent + "(" + relName + ".filter)->exp[" + str(i) + "].index    = " + str(col2index_in_joinRes(left_col)) + ";"
                print >>fo, indent + "(" + relName + ".filter)->exp[" + str(i) + "].secondIndex    = " + str(col2index_in_joinRes(right_col)) + ";"

                print >>fo, indent + "(" + relName + ".filter)->exp[" + str(i) + "].relation = " + "DISTANCE_LTH_VEC;"

                print >>fo, indent + "(" + relName + ".filter)->exp[" + str(i) + "].parameter = " + str(exp.parameter_list[1].cons_value) + ";"
        ...
        if jn.project_func_list is not None and jn.project_func_list != []:
            print("Pass project function to tableScan")
            # pass project to tableScan
            # function input_column output_column
            print >>fo, indent + relName + ".project = (struct projectFunction *)host_mempool.alloc(sizeof(struct projectFunction));"
            print >>fo, indent + relName + ".projectNum =" + str(len(jn.project_func_list)) + ";"
            print >>fo, indent + "(" + relName + ".project)->projectNum =" + str(len(jn.project_func_list)) + ";"
            print >>fo, indent + "(" + relName + ".project)->exp = (struct projectExp*)host_mempool.alloc(sizeof(struct projectExp) *" + str(len(jn.project_func_list)) + ");"
        for i in range(len(jn.project_func_list)):
            exp = jn.project_func_list[i]
            if exp.func_name == "DISTANCE":
                if isinstance(exp.parameter_list[1], ystree.YFuncExp): # "DISTANCE(VECTOR, EXTRACT(PATH))"
                    # get search vec by EXTRACT(PATH)
                    print("-----------------------start image feature extraction---------------------")
                    current_directory = os.getcwd()
                    print(current_directory)
                    searchVec = subprocess.check_output(['python3', '../../vector_src/init_vector/frames_feature_extraction.py', \
                                                         str(exp.parameter_list[1].parameter_list[0].cons_value), '../../vector_src/init_vector/features'])
                    # initial project function
                    print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].index    = " + str(exp.parameter_list[0].column_name) + ";"
                    print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].func     = " + "DISTANCE" + ";"
                    print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].funcType = " + "0" + ";" # extract feature by EXTRACTION(path)
                    print >>fo, indent + "strcpy((" + relName + ".project)->exp[" + str(i) + "].vecPath, \"../../vector_src/init_vector/features\");"
                    print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].limit     = " + str(main.limit) + ";"
                elif isinstance(exp.parameter_list[1], ystree.YConsExp): # "DISTANCE(VECTOR, PATH)"
                    # get search vec by PATH
                    # initial project function
                    print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].index    = " + str(exp.parameter_list[0].column_name) + ";"
                    print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].func     = " + "DISTANCE" + ";"
                    print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].funcType = " + "1" + ";" # get feature from path
                    print >>fo, indent + "strcpy((" + relName + ".project)->exp[" + str(i) + "].vecPath, \"" + str(exp.parameter_list[1].cons_value) + "\");"
                    print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].limit     = " + str(main.limit) + ";"
                elif isinstance(exp.parameter_list[1], ystree.YRawColExp): # "DISTANCE(v1, v2)"
                    print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].index    = " + str(exp.parameter_list[0].column_name) + ";"
                    print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].secondIndex    = " + str(exp.parameter_list[0].column_name + 1) + ";"
                    print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].func     = " + "DISTANCE" + ";"
                    print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].funcType = " + "2" + ";"
            elif exp.func_name == "KNN_AVG_DISTANCE":
                print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].index    = " + str(exp.parameter_list[0].column_name) + ";"
                print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].secondIndex    = " + str(exp.parameter_list[1].column_name) + ";"
                print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].func    = " + "KNN_AVG_DISTANCE" + ";"

        project = 0
        for i in range(0, len(jn.project_func_list)):
            # DISTANCE(V1, V2)
            if ystree.isDistanceTwoCol(jn.project_func_list[i]):
                project = 1
                print >>fo, indent + relName + ".projectPart = 1;"

        if project == 1:
            if HostMempool == 1:
                print >>fo, indent + relName + ".projectIndex = (int *)host_mempool.alloc(sizeof(int) * " + str(len(jn.func_list)) + ");"
                print >>fo, indent + relName + ".projectNum = " + str(len(jn.project_func_list)) + ";"
            else:
                print >>fo, indent + relName + ".projectIndex = (int *)malloc(sizeof(int) * " + str(len(jn.func_list)) + ");"
                print >>fo, indent + relName + ".projectNum = " + str(len(jn.project_func_list)) + ";"
                print >>fo, indent + "CHECK_POINTER(" + relName + ".projectIndex);"
                

            offset = 0
            for i in range(0, len(jn.func_list)):
                if not ystree.isDistanceTwoCol(jn.func_list[i])\
                    and not(isinstance(jn.func_list[i], ystree.YFuncExp) and ystree.isDistanceTwoCol(jn.func_list[i].parameter_list[0])):
                    print >>fo, indent + relName + ".projectIndex[" + str(i) + "] = " + str(i+offset) + ";"
                else:
                    project_index = 0
                    for j in range(len(jn.project_func_list)):
                        if jn.project_func_list[j].func_list_index == i:
                            project_index = j
                            break
                        else:
                            exit(1)
                    print >>fo, indent + relName + ".projectIndex[" + str(i) + "] = " + str(100 + project_index) + ";"
                    offset += 1
        ...
        if CODETYPE == 0:
            ...
            if DeviceMempool == 1:
                ...
                print >>fo, indent + baseIndent + "gpu_inner_mp.resize(gpu_inner_mp.usedsize() + dev_memsize);"
                print >>fo, indent + "char *origin_pos = gpu_inner_mp.freepos();"
                print >>fo, indent + "joinRes = tableScan(&" + relName + ", &pp, " + h_mp + ", " + d_inner_mp + ", " + d_res_mp + ", NULL, NULL, NULL, NULL);"
                print >>fo, indent + "gpu_inner_mp.freeto(origin_pos);\n"
            else:
                print >>fo, indent + resName + " = tableScan(&" + relName + ", &context, &pp);"
...
def generate_code_for_a_table_node(fo, indent, lvl, tn):
    ...
    if HostMempool == 1:
        ...
        print >>fo, indent + tnName + "->attrName = (char **)host_mempool.alloc(sizeof(char *) * " + str(totalAttr) + ");\n"
    ...
    for i in range(0, totalAttr):
        ...
        print >>fo, indent + tnName + "->attrName[" + str(i) + "] = \"" + str(tnName) + str(i) + "\";"
    ...
    if tn.where_condition is not None or (tn.project_func_list is not None and tn.project_func_list != []):
        relName = tn.table_name.lower() + "Rel"
        print >>fo, indent + "struct scanNode " + relName + ";"

    if tn.project_func_list is not None and tn.project_func_list != []:
        print("Pass project function to tableScan")
        # pass project to tableScan
        # function input_column output_column
        # print >>fo, indent + relName + ".tn = " + tnName + ";"
        print >>fo, indent + relName + ".project = (struct projectFunction *)host_mempool.alloc(sizeof(struct projectFunction));"
        print >>fo, indent + relName + ".projectNum = " + str(len(tn.project_func_list)) + ";"
        print >>fo, indent + "(" + relName + ".project)->exp = (struct projectExp*)host_mempool.alloc(sizeof(struct projectExp) *" + str(len(tn.project_func_list)) + ");"
        for i in range(len(tn.project_func_list)):
            exp = tn.project_func_list[i]
            if exp.func_name == "DISTANCE":
                if isinstance(exp.parameter_list[1], ystree.YFuncExp): # "DISTANCE(VECTOR, EXTRACT(PATH))"
                    # get search vec by EXTRACT(PATH)
                    print("-----------------------start image feature extraction---------------------")
                    ...
                    searchVec = subprocess.check_output(['python3', '../../vector_src/init_vector/frames_feature_extraction.py', \
                                                         str(exp.parameter_list[1].parameter_list[0].cons_value), '../../vector_src/init_vector/features'])
                    # initial project function
                    print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].index    = " + str(indexList.index(exp.parameter_list[0].column_name)) + ";"
                    print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].func     = " + "DISTANCE" + ";"
                    print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].funcType = " + "0" + ";" # extract feature by EXTRACTION(path)
                    print >>fo, indent + "strcpy((" + relName + ".project)->exp[" + str(i) + "].vecPath, \"../../vector_src/init_vector/features\"" + ");"
                    print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].limit     = " + str(main.limit) + ";"
                    ...
                    print >>fo, indent + "strcpy((" + relName + ".project)->exp[" + str(i) + "].vecPath, \"" + str(exp.parameter_list[1].cons_value) + "\");"
                    print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].limit     = " + str(main.limit) + ";"
            elif exp.func_name == "OUTLIER_SCORE":
                print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].index    = " + str(indexList.index(exp.parameter_list[0].column_name)) + ";"
                print >>fo, indent + "(" + relName + ".project)->exp[" + str(i) + "].func    = " + "OUTLIER_SCORE" + ";"
    ...
    if tn.where_condition is not None:
        ...
        get_where_attr(tn.where_condition.where_condition_exp, whereList, relList, conList, extractionList, extractionPath)
        newWhereList = []
        whereLen = count_whereList(whereList, newWhereList)
    ...
    if tn.where_condition is not None or (tn.project_func_list is not None and tn.project_func_list != []):
        if CODETYPE == 0:
            ...
            if DeviceMempool == 1:
                ...
                print >>fo, indent + "char *origin_pos = gpu_inner_mp.freepos();"
            if len(indices) > 0:
                print >>fo, indent + resName + " = tableScan(&" + relName + ", &pp, " + h_mp + ", " + d_inner_mp + ", "  + d_res_mp + ", " + indices[0] + ", " + los[0] + " + tupleid, " + his[0] + " + tupleid, NULL);"
            else:
                print >>fo, indent + resName + " = tableScan(&" + relName + ", &pp, " + h_mp + ", " + d_inner_mp + ", "  + d_res_mp + ", NULL, NULL, NULL, NULL);"
            if DeviceMempool == 1:
                print >>fo, indent + "gpu_inner_mp.freeto(origin_pos);\n"
...
def generate_code_for_loading_a_table(fo, indent, t_name, c_list):
    ...
    for i in range(0, totalAttr):
        ...
        print >>fo, indent + tnName + "->attrName[" + str(i) + "] = \"" + str(t_name) + str(colIndex) + "\";"
