...
cd ..
python2 translate.py vector_src/sql/deep.sql vector_src/schema/vector.schema # init driver.cu (physical query plan)
cd src/cuda
sed -i "458i\\
    printf(\"deepres\\\n\");\\
...
for ((i=0; i<$1; i++)); do
    ...
    python3 init_deep_search_vector.py $i
    cd ../src/cuda
