...
struct tableNode * orderBy(struct orderByNode * odNode, struct statistic *pp){
    ...
    // char disArray[4096];
    // char intArray[4096];
    // printf("[orderby.cu] the type of result[0] :%d\n", res->attrType[0]);
    // printf("[orderby.cu] the type of result[1] :%d\n", res->attrType[1]);
    // cudaMemcpy(disArray, odNode->table->content[1], sizeof(float) * 19, cudaMemcpyDeviceToHost);
	// for(int i = 0; i < 19; i++){
	// 	printf("[orderby.cu] distances[%d] = %f\n", i, ((float *)disArray)[i]);
	// }
    // cudaMemcpy(intArray, odNode->table->content[0], sizeof(char) * 23 * 19, cudaMemcpyDeviceToHost);
    // for(int i = 0; i < 19; i++){
    ...
    // printf("[orderby.cu] the type of result[0] :%d\n", res->attrType[0]);
    // printf("[orderby.cu] the type of result[1] :%d\n", res->attrType[1]);
    // cudaMemcpy(disArray, result[1], sizeof(float) * 19, cudaMemcpyDeviceToHost);
	// for(int i = 0; i < 19; i++){
	// 	printf("[orderby.cu] distances[%d] = %f\n", i, ((float *)disArray)[i]);
	// }
    // cudaMemcpy(intArray, result[0], sizeof(char) * 23 * 19, cudaMemcpyDeviceToHost);
    // for(int i = 0; i < 19; i++){
    //     printf("video_name[%d] = ", i);
	// 	for(int j = 0; j < 23; j++){
    //         printf("%c", disArray[i*23 + j]);
    ...
    // printf("[driver.cu] the type of result[1] :%d\n", res->attrType[1]);
    // memcpy(intArray, res->content[0], sizeof(int) * 100);
    // for(int i = 0; i < 100; i++){
    //     printf("%d\n", intArray[i]);
	// }

    printf("[orderby] res->tupleSize = %d\n", res->tupleSize);
    return res;
}
