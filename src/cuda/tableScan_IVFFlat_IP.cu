...
__global__ void setIndicesKernel(int* globalFilter, faiss::idx_t* nns, size_t k) {
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < k and nns[idx] >= 0) {
    ...
    */
    for (int i = 0; i < disFilterCount; i++) {
        ...
        if(faiss_index != NULL){
            ...
            double search_t = (annsSearchEnd.tv_sec -  annsSearchStart.tv_sec)* BILLION + annsSearchEnd.tv_nsec - annsSearchStart.tv_nsec;
        ...
        else
        {
            ...
            // faiss::gpu::GpuIndexIVFFlat index(&resources, d, ncentroids, faiss::METRIC_L2, config);
            faiss::gpu::GpuIndexIVFFlat index(&resources, d, ncentroids, faiss::METRIC_INNER_PRODUCT, config);
            index.nprobe = 256;
        ...
        // }
        faiss::idx_t* nns_dev;
        CUDA_SAFE_CALL_NO_SYNC(cudaMalloc((void**)&nns_dev, sizeof(faiss::idx_t) * k));
        CUDA_SAFE_CALL_NO_SYNC(cudaMemcpy(nns_dev, nns.data(), sizeof(faiss::idx_t)*k, cudaMemcpyHostToDevice));
                ...
                if(rel == KNN){
                    ...
                    else
                    {
                        ...
                        if(sn->tn->tupleNum < 1000){
                            ...
                            // searching...
                            clock_gettime(CLOCK_REALTIME,&annsSearchStart);
                            gpuIndex.search(nq, searchV.data(), k, dis.data(), nns.data());
                            clock_gettime(CLOCK_REALTIME,&annsSearchEnd);
