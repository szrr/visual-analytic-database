...
int main(int argc, char ** argv){
    ...
    test_videoTable = (struct tableNode *)host_mempool.alloc(sizeof(struct tableNode));
    {
        ...
        for(int i = 0; i < blockTotal; i++){
            ...
            _test_video_table->dataPos[0] = MEM;
            outFd = open("TEST_VIDEO0", O_RDONLY);
            ...
            _test_video_table->dataPos[1] = MEM;
            ...
            _test_video_table->dataPos[2] = MEM;
    ...
    train_videoTable = (struct tableNode *)host_mempool.alloc(sizeof(struct tableNode));
    {
        ...
        for(int i = 0; i < blockTotal; i++){
            ...
            _train_video_table->dataPos[0] = MEM;
            outFd = open("TRAIN_VIDEO0", O_RDONLY);
            offset = i * sizeof(struct columnHeader) + tupleOffset * 76;
            lseek(outFd, offset, SEEK_SET);
            read(outFd, &header, sizeof(struct columnHeader));
    ...
    struct timespec vec_s, vec_e;
    clock_gettime(CLOCK_REALTIME, &vec_s);
...

    {
        ...
        (selectProjectRel.filter)->exp[0].relation = GTH_VEC;
    ...
    clock_gettime(CLOCK_REALTIME, &vec_e);
    double vec_time = (vec_e.tv_sec -  vec_s.tv_sec)* BILLION + vec_e.tv_nsec - vec_s.tv_nsec;

    struct timespec other_s, other_e;
    clock_gettime(CLOCK_REALTIME, &other_s);
    ...
    clock_gettime(CLOCK_REALTIME, &other_e);
    double other_time = (other_e.tv_sec -  other_s.tv_sec)* BILLION + other_e.tv_nsec - other_s.tv_nsec;

    clock_gettime(CLOCK_REALTIME, &end);
    double timeE = (end.tv_sec -  start.tv_sec)* BILLION + end.tv_nsec - start.tv_nsec;

    printf("vec Time: %lf\n", vec_time/(1000*1000));
    printf("other Time: %lf\n", other_time/(1000*1000));
